...
namespace AirportTicketBookingSystem.Application.Services
{
    public sealed class CsvFlightImportService
    {
        ...
        public async Task<FlightImportResult> ImportAsync(string filePath)
        {
            ...
            for (var i = 1; i < lines.Length; i++)
            {
                ...
                if (columns.Length != 13)
                {
                    result.Errors.Add(new FlightImportError
                    {
                        RowNumber = rowNumber,
                        FieldName = "Row",
                        ErrorMessage = "Invalid column count. Expected 13 columns."
                    });
            ...
            result.ImportedCount = flights.Count;

            return result;
        }

        private static void ValidateRequired(FlightImportResult result, int rowNumber, string fieldName, string value)
        {
        ...
        private static void AddError(FlightImportResult result, int rowNumber, string fieldName, string errorMessage)
        {
            result.Errors.Add(new FlightImportError
            {
                RowNumber = rowNumber,
